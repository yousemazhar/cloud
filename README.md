...
npm test             # vitest (server workspace) — 59 tests across 14 files
...
| Notifications | [`AssignmentNotifier`](server/src/services/notifications.ts) | `NoopNotifier` | `SnsNotifier` |
...
files. `npm test` from the repo root runs all 59 tests.

## AWS deployment (out of scope for app commits)

The deployment workstream provisions the infrastructure the app consumes: VPC across 2 AZs,
...
widgets and alarms. The app code in this repo is the producer side; the AWS impls are coded
and unit-tested, but will be exercised end-to-end once the infra exists.
